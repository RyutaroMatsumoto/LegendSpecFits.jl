...
* `gamma_lines`: array of gamma lines in energy units
* `window_sizes`: array of tuples with left and right window sizes around the gamma lines
* `kwargs...` that are passed to `simple_calibration_gamma`
# outputs:
* `result`, `report` see `simple_calibration_gamma` for details.
"""
...
* result containing: 
    ...
    * `peak_guess`: estimated peak used for simple calibration (full-energy peak FEP for th228)
    * `peakbinwidths`: bin widths for peak-wise histograms 
    * `peakhists`: array of histograms around the calibration lines
    * `peakstats`: array of statistics for the calibration line fits
* report that can be plotted via `plot(report)`
"""
